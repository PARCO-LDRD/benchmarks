...
#pragma omp begin declare adaptation feature (n) model_name(mymodel) variant(serial,ompcpu,ompgpu) model (dtree)

#pragma omp metadirective \
    when (user={adaptation(mymodel==ompgpu)}: target data map (to: p[0:n]) map(alloc: e[0:n])) 
  {
...
#pragma omp metadirective \
  ...
  default(target teams distribute parallel for thread_limit(256))
      for (int i = 0; i < n; i++) {
...
#pragma omp metadirective \
  when (user={adaptation(mymodel==ompgpu)}: target )
      for (int i = 1; i < n; i++) e[0] += e[i];

#pragma omp metadirective \
        when (user={adaptation(mymodel==ompgpu)}: target update from (e[0:1]) )
      kenergy_ = 0.5 * e[0];
      e[0] = 0;
