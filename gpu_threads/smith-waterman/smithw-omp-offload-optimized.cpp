...
void  similarityScore(long long int ai, long long int aj, int*H, int*P, long long int* maxPos);
...
//  printPredecessorMatrix(P);   // unicode not supported on lassen
  ...
  if (useBuiltInData)
  {
    printf ("Verifying results using the builtinIn data: H[n*m-1]= %d \n", H[n*m-1]);
 ...
 */
void  similarityScore(long long int ai, long long int aj, int*H, int*P, long long int* maxPos) // a critical section is used inside
{
  int up, left, diag;
