...
#pragma omp metadirective \
  ...
  when(user={adaptation(by_nelements==threads_128)} : \
    target teams distribute parallel for map(tofrom:maxPos) thread_limit(128)) \
  when(user={adaptation(by_nelements==threads_256)} : \
    target teams distribute parallel for map(tofrom:maxPos) thread_limit(256)) \
  when(user={adaptation(by_nelements==threads_512)} : \
...
#pragma omp end declare adaptation model_name(by_nelements)
