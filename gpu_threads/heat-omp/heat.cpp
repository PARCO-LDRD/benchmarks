...
#pragma omp metadirective \
  ...
  when(user={adaptation(by_grid_size==threads_128)} : \
    target teams distribute parallel for simd collapse(2) thread_limit(128)) \
  when(user={adaptation(by_grid_size==threads_256)} : \
    target teams distribute parallel for simd collapse(2) thread_limit(256)) \
  when(user={adaptation(by_grid_size==threads_512)} : \
    target teams distribute parallel for simd collapse(2) thread_limit(512)) \
  when(user={adaptation(by_grid_size==threads_1024)} : \
    target teams distribute parallel for simd collapse(2) thread_limit(1024))
    for (int j = 0; j < n; ++j) {
      for (int i = 0; i < n; ++i) {
