...
#pragma omp metadirective \
  when(user={adaptation(by_lookups==threads_64)} : \
    target teams distribute parallel for) \
  when(user={adaptation(by_lookups==threads_128)} : \
    target teams distribute parallel for num_teams(1171875/4) num_threads(512)) \
  when(user={adaptation(by_lookups==threads_256)} : \
    target teams distribute parallel for num_teams(4096*3) num_threads(1024)) \
  when(user={adaptation(by_lookups==threads_512)} : \
    target teams distribute parallel for num_teams(4096*4)) \
  when(user={adaptation(by_lookups==threads_1024)} : \
    target teams distribute parallel for num_teams(4096*5))
	for( int i = 0; i < in.lookups; i++ )
	{
	  if(omp_get_thread_num() == 0 && omp_get_team_num() == 0)
	    printf("Num teams: %d num threads %d\n", omp_get_num_teams(), omp_get_num_threads());
