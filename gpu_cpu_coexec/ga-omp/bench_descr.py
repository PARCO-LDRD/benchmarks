...
class Benchmark(BaseBenchmark):
  ...
  def visualize(self, df, outfile, sizes):
    ...
    g = sns.FacetGrid(df, col='System', hue='Policy')

    def plot_func(data, *args, **kwargs):
      if (data['Policy']=='oracle').all():
        print('LINEPLOT')
        return sns.lineplot(data=data, *args, **kwargs, marker='x', markeredgecolor=None, markersize=5, ls='--')
      else:
        print('SCATTERPLOT')
        return sns.scatterplot(data=data, *args, **kwargs, marker='o', s=100)

    g.map_dataframe(plot_func, x='Input', y='Execution time (s)').add_legend()

    #g.map_dataframe(sns.lineplot, 'Input', 'Execution time (s)', marker='+').add_legend()
