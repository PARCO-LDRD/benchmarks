...
#pragma omp begin declare adaptation feature(num_atoms) model_name(by_num_atoms) \
  variants(cpu, gpu) model(dtree)

#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : target enter data map(to: idxu_block[0:jdim], \
...
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
      target update to(rij[0:num_atoms*num_nbor*3]))
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
      target update to(rcutij[0:num_atoms*num_nbor]))
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
      target update to(wj[0:num_atoms*num_nbor]))
...
#pragma omp metadirective \
  ...
  when(user={adaptation(by_num_atoms==cpu)} : \
    parallel for default(none) shared(ulisttot, num_atoms, idxu_max))
...
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
    target teams distribute parallel for) \
  when(user={adaptation(by_num_atoms==cpu)} : \
    parallel for default(none) shared(ulisttot, wself, idxu_block, num_atoms, twojmax))
    for (int natom = 0; natom < num_atoms; natom++) {
      for (int j = 0; j <= twojmax; j++) {
...
#pragma omp metadirective \
  ...
  when(user={adaptation(by_num_atoms==cpu)} : \
    parallel for collapse(2) default(none) \
...
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
    target teams distribute parallel for) \
  when(user={adaptation(by_num_atoms==cpu)} : \
    parallel for default(none) shared(num_atoms, idxdu_max, ylist))
...
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
    target teams distribute parallel for collapse(2)) \
  when(user={adaptation(by_num_atoms==cpu)} : \
    parallel for collapse(2) default(none) shared(idxz,                \
    idxzbeta,            \
    idxcg_block,         \
    idxdu_block,         \
    idxu_block,          \
...
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
    target teams distribute parallel for collapse(2)) \
  when(user={adaptation(by_num_atoms==cpu)} : \
    parallel for collapse(2) default(none) shared(rij, wj, rcutij, rootpqarray, dulist, ulist, \
...
#pragma omp metadirective \
  when(user={adaptation(by_num_atoms==gpu)} : \
    target update from(dedr[0:num_atoms * num_nbor * 3]))
    end = system_clock::now();
    elapsed = end - start;
...
#pragma omp end declare adaptation model_name(by_num_atoms)
