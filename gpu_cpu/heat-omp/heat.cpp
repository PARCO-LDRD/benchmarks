...
#pragma omp metadirective \
  when(user={adaptation(by_grid_size==gpu)} : \
      target enter data map(to: u[0:n*n], u_tmp[0:n*n]))
{
  // Set the initial value of the grid under the MMS scheme
#pragma omp metadirective \
  when(user={adaptation(by_grid_size==gpu)} : \
      target teams distribute parallel for simd collapse(2) thread_limit(block_size)) \
  when(user={adaptation(by_grid_size==cpu)} : \
...
#pragma omp target exit data map(from: u[0:n*n], u_tmp[0:n*n]) 
