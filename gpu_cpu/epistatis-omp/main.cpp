...
#pragma omp metadirective \
  when(user={adaptation(by_size==gpu)} : \
      target enter data map(to: dev_data_zeros[0:num_snp * PP_zeros * 2], \
...
#pragma omp metadirective \
    when(user={adaptation(by_size==gpu)} : \
        target update to (dev_scores[0:num_snp*num_snp]))

#pragma omp metadirective \
  when(user={adaptation(by_size==gpu)} : \
    target teams distribute parallel for thread_limit(block_snp)) \
...
#pragma omp metadirective \
  when(user={adaptation(by_size==gpu)} : \
      target update from (dev_scores[0:num_snp*num_snp]))
