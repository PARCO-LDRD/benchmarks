  ...
  template<typename OperatorType,
    ...
    typename Matvec>
          ...
          timer_type* my_cg_times)
      {
        ...
        ScalarType one = 1.0;
        ScalarType zero = 0.0;
        ScalarType x_size = x.coefs.size();
        ScalarType A_packed_size= A.packed_cols.size();
...
#pragma omp begin declare adaptation model_name(miniFE) feature(x_size,A_packed_size)  variants(single, CPU, GPU) model(dtree)
...
#pragma omp end declare adaptation model_name(miniFE)
