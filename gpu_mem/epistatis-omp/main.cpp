...
int main(int argc, char **argv)
{
  ...
  omp_memspace_handle_t  pinned_memspace = llvm_omp_target_host_mem_space;

  omp_alloctrait_t       xy_traits[1]= {{omp_atk_alignment, 64}};
  omp_allocator_handle_t pinned_alloc = omp_init_allocator(pinned_memspace,0,xy_traits);
  omp_allocator_handle_t aligned_alloc = omp_init_allocator(omp_default_mem_space,1,xy_traits);
  ...
  unsigned int bin_data_ones_trans[num_snp* PP_ones *2]; // = mem_alloc<unsigned int>(64, num_snp * PP_ones * 2);
  unsigned int bin_data_zeros_trans[num_snp*PP_zeros*2];
  float dev_scores[num_snp*num_snp];


  // transpose the binary data structures
  // unsigned int* bin_data_ones_trans = mem_alloc<unsigned int>(64, num_snp * PP_ones * 2);
  //
#pragma omp begin declare adaptation feature(num_snp) model_name(memory_protocol) \
  variants(host, pinned) model(dtree)

#pragma omp metadirective when(user={adaptation(memory_protocol==host)} : allocate(bin_data_ones_trans, bin_data_zeros_trans, dev_scores) allocator(aligned_alloc)) \
                          when(user={adaptation(memory_protocol==pinned)} : allocate(bin_data_ones_trans, bin_data_zeros_trans, dev_scores) allocator(pinned_alloc))
...
#pragma omp target enter data map(to: dev_data_zeros[0:num_snp * PP_zeros * 2], \
...
#pragma omp end declare adaptation model_name(memory_protocol)
