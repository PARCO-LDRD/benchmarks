...
#pragma omp begin declare adaptation feature(total_sites) model_name(by_total_sites) \
  variants(cpu, gpu) model(dtree)
  // allocate offsets for device gathers and copy to shared buffers
  //
#pragma omp metadirective \
  when(user={adaptation(by_total_sites==gpu)} : \
      target enter data map (to: d_src[0:total_sites*1], \
...
#pragma omp metadirective \
  when(user={adaptation(by_total_sites==gpu)} : \
    target teams distribute parallel for thread_limit(1)) \
  when(user={adaptation(by_total_sites==cpu)} : \
      parallel for)
...
#pragma omp metadirective \
    when(user={adaptation(by_total_sites==gpu)} : \
      target teams distribute parallel for thread_limit(wgsize)) \
    when(user={adaptation(by_total_sites==cpu)} : \
      parallel for)
    for (size_t mySite = 0; mySite < total_even_sites; mySite++) {
      su3_vector v;
...
#pragma omp metadirective \
  when(user={adaptation(by_total_sites==gpu)} : target exit data map(from: d_dst[0:total_sites],\
        d_fatbck[0:total_sites*4],\
        d_lngbck[0:total_sites*4]))
}

#pragma omp end declare adaptation model_name(by_total_sites)
