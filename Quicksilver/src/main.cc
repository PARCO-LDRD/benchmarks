...
int main(int argc, char** argv)
{
  ...
  omp_target_free( mcco , omp_get_default_device());
#else
  cudaFree( mcco );
#endif
#else
        ...
        if ( numParticles != 0 )
        {
          ...
          switch (execPolicy)
          {
            ...
            case gpuWithOpenMP:
              {
#endif
#pragma omp begin declare adaptation feature (numParticles) model_name(mymodel) variants(ompcpu,ompgpu) model (dtree)
